...
function darkliteInit() {
  ...
  if (document.querySelector('[data-fls-darklite-time]')) {
    ...
    let customRange = document.querySelector('[data-fls-darklite-time]').dataset
      .flsDarkliteTime;
  ...
  function setThemeClass() {
    htmlBlock.setAttribute(
      `data-fls-darklite-${saveUserTheme ? saveUserTheme : userTheme}`,
      ''
    );
  }
  // Додаємо клас теми
  ...
  function changeTheme(saveTheme = false) {
    let currentTheme = htmlBlock.hasAttribute('data-fls-darklite-light')
      ? 'light'
      : 'dark';
    let newTheme;
